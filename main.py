...
game_over_music.set_volume(0.4)#Гучність
...
def draw_about(window):
    ...
    text_6 = fonts2.render("See you soon! Bye!",True,WHITE)
...
#Екран "about"
    return_about_btn = Button("R E T U R N",290,500,150,150,WHITE,60)
...
#Обробка подій на кліці мишки
                if event.type == MOUSEBUTTONDOWN:
                    ...
                    if menu_pause_btn.x <= x <= menu_pause_btn.x + menu_pause_btn.width and menu_pause_btn.y <= y <= menu_pause_btn.y + menu_pause_btn.height:
                        screen = 'menu'
                        board = create_board(points) #поле оновлюється
